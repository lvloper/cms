    ...
    protected function setUp(): void
    {
        ...
        $this
            ...
            ->action(function (TiptapEditor $component, array $arguments, $data) {
                $uploadedSrc = $data['src'] ?? null;
                $existingSrc = $arguments['src'] ?? null;

                if ($this->isUsingCurator()) {
                    $source = $existingSrc;
                } elseif ($uploadedSrc) {
                    if (config('filament-tiptap-editor.use_relative_paths')) {
                        $source = Str::of($uploadedSrc)
                            ->replace(config('app.url'), '')
                            ->ltrim('/')
                            ->prepend('/');
                    } else {
                        $source = str_starts_with($uploadedSrc, 'http')
                            ? $uploadedSrc
                            : Storage::disk(config('filament-tiptap-editor.disk'))->url($uploadedSrc);
                    }
                } else {
                    $source = $existingSrc;
                }
