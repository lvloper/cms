@php
    ...
    $gridDefault = $gridColumns['default'] ?? 6;
    $gridSm = $gridColumns['sm'] ?? $gridDefault;
    $gridMd = $gridColumns['md'] ?? $gridSm;
    $gridLg = $gridColumns['lg'] ?? $gridMd;
@endphp
...
<x-dynamic-component :component="$getFieldWrapperView()" :field="$field">
    <div x-data="{
        ...
        totalIcons: 0,
        currentPage: 1,
        perPage: @js(config('filament-icon-picker.icons_per_page', 100)),
        iconCache: {},
    ...
    }" wire:ignore class="fi-fo-icon-picker" style="--fi-icon-grid-default: {{ $gridDefault }}; --fi-icon-grid-sm: {{ $gridSm }}; --fi-icon-grid-md: {{ $gridMd }}; --fi-icon-grid-lg: {{ $gridLg }};">
        ...
        <x-filament::input.wrapper :disabled="$isDisabled" :valid="!$errors->has($statePath)" class="fi-fo-icon-picker-trigger cursor-pointer"
            x-on:click.stop="openModal()">
            <button type="button" @disabled($isDisabled)
                class="fi-icon-picker-trigger-button flex min-h-9 w-full items-center gap-x-2 rounded-lg py-1.5 ps-3 pe-3 text-start text-sm leading-6 text-gray-950 focus:ring-0 focus:outline-none dark:text-white">
                ...
                <template x-if="state">
                    <span x-on:click.stop="clearSelection()"
                        class="fi-icon-picker-clear-button shrink-0 flex items-center justify-center rounded h-5 w-5 text-gray-400 hover:text-gray-500 dark:text-gray-500 dark:hover:text-gray-400 hover:bg-gray-100 dark:hover:bg-white/10"
                ...
                {{-- Dropdown Icon --}}
                <span class="fi-icon-picker-dropdown-icon shrink-0 text-gray-400 dark:text-gray-500">
        ...
        <template x-teleport="body">
            <div x-show="isOpen" x-transition:enter="ease-out duration-300" x-transition:enter-start="opacity-0"
                ...
                <div x-show="isOpen" x-trap="isOpen" x-transition:enter="ease-out duration-300"
                    ...
                    <div class="px-6 pb-4">
                        <div class="fi-icon-picker-toolbar flex gap-3">
                            {{-- Provider Select --}}
                            <template x-if="showSetFilter && availableSets.length > 1">
                                <select x-model="selectedSet" x-on:change="handleSetChange()"
                                    class="fi-select-input fi-icon-picker-set-select block w-48 rounded-lg border-none bg-gray-50 py-2 pe-8 ps-3 text-sm text-gray-950 ring-1 ring-inset ring-gray-950/10 transition duration-75 focus:bg-white focus:ring-2 focus:ring-inset focus:ring-primary-600 dark:bg-white/5 dark:text-white dark:ring-white/20 dark:focus:bg-white/10 dark:focus:ring-primary-500">
                            ...
                            <div x-show="isSearchable" class="relative flex-1">
                                <div class="fi-icon-picker-search-icon pointer-events-none absolute inset-y-0 start-0 flex items-center ps-3">
                                ...
                                <input type="text" x-ref="searchInput" x-model="search" x-on:input="handleSearchInput()"
                                    ...
                                    class="fi-input fi-icon-picker-search-input block w-full rounded-lg border-none bg-gray-50 py-2 pe-3 ps-10 text-sm text-gray-950 ring-1 ring-inset ring-gray-950/10 transition duration-75 placeholder:text-gray-400 focus:bg-white focus:ring-2 focus:ring-inset focus:ring-primary-600 dark:bg-white/5 dark:text-white dark:ring-white/20 dark:placeholder:text-gray-500 dark:focus:bg-white/10 dark:focus:ring-primary-500" />
                            </div>
                        </div>
                    </div>
                    ...
                    <div x-ref="iconsContainer" class="fi-modal-content overflow-y-scroll px-6 pb-4"
                        ...
                        {{-- Loading State --}}
                        <div x-show="isLoading && icons.length === 0" class="fi-icon-picker-loading-state flex items-center justify-center py-12">
                        ...
                        <div x-show="!isLoading && icons.length === 0"
                            class="fi-icon-picker-empty-state flex flex-col items-center justify-center py-12 text-center">
                        ...
                        {{-- Icons Grid --}}
                        <div x-show="icons.length > 0" class="fi-icon-picker-grid grid grid-cols-5 sm:grid-cols-7 md:grid-cols-9 lg:grid-cols-10 gap-3">
    ...
    <style>
        .fi-fo-icon-picker .fi-icon-picker-trigger-button,
        .fi-fo-icon-picker .fi-icon-picker-preview,
        .fi-fo-icon-picker .fi-icon-picker-preview svg,
        .fi-fo-icon-picker .fi-icon-picker-icon,
        .fi-fo-icon-picker .fi-icon-picker-icon svg {
            flex-shrink: 0;
        }
        ...
        .fi-fo-icon-picker .fi-icon-picker-preview,
        .fi-fo-icon-picker .fi-icon-picker-icon,
        .fi-fo-icon-picker .fi-icon-picker-empty-state svg,
        .fi-fo-icon-picker .fi-icon-picker-loading-state svg {
            display: flex;
            align-items: center;
        ...
        .fi-fo-icon-picker .fi-icon-picker-empty-state svg,
        .fi-fo-icon-picker .fi-icon-picker-loading-state svg,
        .fi-fo-icon-picker .fi-icon-picker-search-icon svg,
        .fi-fo-icon-picker .fi-icon-picker-dropdown-icon svg,
        .fi-fo-icon-picker .fi-icon-picker-clear-button svg,
        .fi-fo-icon-picker .fi-icon-btn svg {
            width: 100% !important;
        ...
        .fi-icon-picker-modal .fi-modal-close-overlay {
            position: fixed;
            inset: 0;
        }

        .fi-icon-picker-modal .fi-modal-window {
            position: relative;
            display: flex;
        ...
        .fi-icon-picker-modal .fi-modal-header,
        .fi-icon-picker-modal .fi-modal-footer,
        .fi-icon-picker-modal .fi-modal-content,
        .fi-icon-picker-modal .fi-icon-picker-loading-state,
        .fi-icon-picker-modal .fi-icon-picker-empty-state {
            display: flex;
        }
        ...
        .fi-icon-picker-modal .fi-icon-picker-grid {
            display: grid;
            grid-template-columns: repeat(var(--fi-icon-grid-default), minmax(0, 1fr));
            gap: 0.75rem;
        }

        .fi-icon-picker-modal .fi-icon-picker-item {
            display: flex;
            align-items: center;
            justify-content: center;
            aspect-ratio: 1 / 1;
        ...
        .fi-icon-picker-modal .fi-icon-picker-toolbar {
            display: flex;
            gap: 0.75rem;
        }

        .fi-icon-picker-modal .fi-icon-picker-set-select {
            min-width: 12rem;
        }

        .fi-icon-picker-modal .fi-icon-picker-search-input {
            width: 100%;
        }

        @media (min-width: 640px) {
            .fi-icon-picker-modal .fi-icon-picker-grid {
                grid-template-columns: repeat(var(--fi-icon-grid-sm), minmax(0, 1fr));
            }
        }

        @media (min-width: 768px) {
            .fi-icon-picker-modal .fi-icon-picker-grid {
                grid-template-columns: repeat(var(--fi-icon-grid-md), minmax(0, 1fr));
            }
        }

        @media (min-width: 1024px) {
            .fi-icon-picker-modal .fi-icon-picker-grid {
                grid-template-columns: repeat(var(--fi-icon-grid-lg), minmax(0, 1fr));
            }
        }
